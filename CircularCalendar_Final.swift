...
struct ContentView: View {
    ...
    func addEvent() {
        ...
        guard startYear == self.selectedYear && endYear == self.selectedYear else {
    ...
    func deleteEvent(event: CalendarEvent) {
        self.events.removeAll(where: { item in
...
struct CircularCalendarView: View {
    ...
    var body: some View {
        Canvas { context, size in
            ...
            let eventArray = Array(self.events.enumerated())
            for (index, event) in eventArray {
                let radiusOffset = CGFloat(index * 20)
    ...
    func createArcPath(center: CGPoint, innerRadius: CGFloat, outerRadius: CGFloat, startAngle: Angle, endAngle: Angle) -> Path {
        return Path { path in
            ...
            path.addArc(center: center, radius: innerRadius, startAngle: endAngle, startAngle: startAngle, clockwise: true)
            path.closeSubpath()
        }
    }
    ...
    func dateToAngle(date: Date) -> Angle {
        ...
        angle = angle - (Double(startMonthDay) / Double(daysInYear) * 360.0)
